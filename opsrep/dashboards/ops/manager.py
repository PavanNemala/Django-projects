...
def get_elasticsearch_cluster_info():
    f = open('/opt/opsrep/dashboards/ops/elastic_search.json')
    elastic_search_json = json.load(f)
    es_stats = dict()
    es_stats.update(elastic_search_json)
    es_dashboards = list()
    es_dashboards.append({
        'name': es_stats['cluster.name'],
        'state': es_stats['cluster.state'],
        'nodes': es_stats["cluster.nodes"],
        'data_nodes': es_stats["cluster.nodes.data"],
        'active_primary_shards': es_stats["active.primary.shards"],
        'active_shards': es_stats["active.shards"] 
    })
    indices = es_stats["indices"]
    indices_list = []
    for i in indices:
        ...
        context = {'es_stats': str(json.dumps(es_dashboards)), 'indices': str(json.dumps(indices)),'indices_list': str(json.dumps(indices_list))}
    return context


def get_cassandra_nodetool_status():
    f = open('/opt/opsrep/dashboards/ops/cassandra_nodetool_status.json')
    cassandra_nodetool_status = json.load(f)
    status_list = []
    status_list.append(cassandra_nodetool_status)
    context = {'nodetool_status': json.dumps(status_list)}
    return context


def get_health_metrics():
    import sqlite3
    conn = sqlite3.connect('/opt/opsrep/health_dashboards.db')
    c = conn.cursor()
        ...
        for item in value:
            ...
            if len(item) ==5:
                inner_list.append({
                    "id": item[0],
                    "name":item[1],
                    "serviceprovider":item[2],
                    "partner":item[3],
                    "email":item[4]
                })

        return_dictionary[key] = inner_list

    return_list = []
    return_list.append(return_dictionary)

    context = {'health_metrics': json.dumps(return_list)}
    return context


def past_incidents():
    f = open('/opt/opsrep/dashboards/ops/past_incidents.json')
    incidents = json.load(f)
    status_list = []
    status_list.append(incidents)
    context = {'past_incidents': json.dumps(status_list)}
    return context
