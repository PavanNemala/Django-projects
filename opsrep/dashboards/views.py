...
def dashboard(request):
    ...
    last_7dates.reverse()
    context =  {'pingdom_stats': properties.pingdom, 'last_7dates': last_7dates}
    context.update(manager.past_incidents())
...
def cassandra(request):
    context = manager.get_cassandra_nodetool_status()
    return render(request, 'views/cassandra.html', context)

def health_metrics(request):
    context = manager.get_health_metrics()
    return render(request, 'views/health_metrics.html', context)
